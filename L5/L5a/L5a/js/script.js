...
requestImages(file) { // Parametern nr används i url:en för den fil som ska läsas in
	...
	request.open("GET","json/" + file + ".json" ,true);
	request.send(null); // Skicka begäran till servern
	request.onreadystatechange = function () { // Funktion för att avläsa status i kommunikationen
		if (request.readyState == 4) // readyState 4 --> kommunikationen är klar
			if (request.status == 200)this2.getImages(request.responseText); // status 200 (OK) --> filen fanns
...
getImages(JSONtext) { // Parametern XMLcode är hela den inlästa XML-koden
	...
	let urlElems = JSON.parse(JSONtext).image; // Alla url-element
	let captionElems = JSON.parse(JSONtext).image; // Alla caption-element
